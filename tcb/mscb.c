...
#define TO_SHORT           10   /* 10 ms */
#define TO_LONG           300   /* 0.3 s */
...
#define RS485_FLAG_LONG_TO   (1<<3)  /* 3s for flash */
...
void debug_log(const char *format, int start, ...)
{
   ...
   if (_debug_flag == 2)
      wr = write(fileno(stdout), line, strlen(line));
   else {
      /* else write to file */
      ...
      wr = write(fh, line, strlen(line));
      close(fh);
   }

   /* squash compiler warning about unused "wr" */
   wr = 0;
   assert(wr == 0);
}
...
int host2ip(char *hostname, char *ip, int size)
{
   struct hostent *he;
   ...
   if (addr_list[0] != NULL) {
      ...
      strlcpy(ip, inet_ntoa(*addr_list[0]), size);
      return 1;
   }
...
static int mscb_usb_timeout = 60; /* 60 ms, must be longer than TO_LONG */
...
void mscb_get_version(char *lib_version, char *prot_version)
{
   ...
   sprintf(prot_version, "%d", MSCB_PROTOCOL_VERSION);
}
...
\********************************************************************/
{
   ...
   dev3[2] = 0;
   /* WDxxx*/
   if (strieq(dev3, "wd") || (atoi(device) > 0 && strchr(device, '.')))
      mscb_fd[index].type = MSCB_TYPE_ETH;
   ...
   if (mscb_fd[index].type == MSCB_TYPE_USB) {
      ...
      for (i = 0; i < mscb_max_retry; i++) {
         ...
         if (n > 0 && (n < 4 || buf[1] != MSCB_PROTOCOL_VERSION)) {
            /* invalid version */
            debug_log("return EMSCB_PROTOCOL_VERSION\n", 0);
            memset(&mscb_fd[index], 0, sizeof(MSCB_FD));
            return EMSCB_PROTOCOL_VERSION;
         }
...
\********************************************************************/
{
   ...
   for (i=0 ; i<1000 ; i++) {
      ...
      if (n >= 4) {
         rev = (buf[2] << 8) + buf[3];
         printf("Found %s, PV %d, Rev. 0x%04X", str, buf[1], rev);
         if (n >= 8) {
