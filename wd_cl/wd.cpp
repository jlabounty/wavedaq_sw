...
void wd_write_reg(GLOBALS *gl, int index, int address, int data)
{
    char cmd[80];
    sprintf(cmd, "llwr %08x %08x", (address<<2), data);
    assert(wd_send(gl, index, 100, cmd, NULL, NULL) > 0);
}

/*-----------------------------------------------------------------------------------------*/

void wd_read_reg(GLOBALS *gl, int index, int address, unsigned int* data, int len)
{
    int size;
    char cmd[80];
    char* str = (char*) malloc (len*26);
    size = sizeof(str);
    sprintf(cmd, "llrd %08x %d", (address<<2), len);
  ...
  //  char buftemp[8];
    ...
    for (int i=0; i<len; i++) {
    /*    for (int j=0; j<8; j++) {
            buftemp[j] = str[i*26 + 17+j];
        }*/
        //data[i] =strtoul(buftemp, NULL, 16);
        data[i] =strtoul(str+17+i*26, NULL, 16);
    }
