...
int cmd_proc_unsplit_args(int max_arg_len, unsigned int argc, char *argv_ptr)
{
  int len;
  unsigned int arg_cnt = argc;
  char* c_ptr = argv_ptr;
    ...
    for (cmd_ptr = *cmd_list_ptr; cmd_ptr->cmd_name != NULL; cmd_ptr++)
    {
      if( fstrcmp(argv[0], cmd_ptr->cmd_name) )
      {
        ...
        if ((argc > 1) && (arg_limit>argc) && (idx<len))
        {
          ...
          cmd_proc_split_args(argv[1], len-idx, arg_limit-1, &argc, argv+1);
          argc++; /* count previously splitted command too */
        }
        if (DBG_ALL) cmd_proc_arg_info(argc, argv);
  ...
  for (cmd_list_ptr = cmd_list; *cmd_list_ptr; cmd_list_ptr++)
  {
    for (cmd_ptr = *cmd_list_ptr; cmd_ptr->cmd_name != NULL; cmd_ptr++)
    {
      if( fstrcmp(argv[0], cmd_ptr->cmd_name) )
      {
        ...
        if( argc > arg_limit )
        {
          cmd_proc_unsplit_args(255, argc-arg_limit+1, argv[arg_limit-1]);
          argc_func = arg_limit;
        }
        if (DBG_ALL) cmd_proc_arg_info(argc_func, argv);
        cmd_ptr->cmd_func_ptr(argc_func, argv);
        return 0;
      }
    }
  }
